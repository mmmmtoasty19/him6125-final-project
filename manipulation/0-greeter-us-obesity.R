...
folder <- "./data-unshared/raw/us-obesity-data"
...
ds_obesity_raw <- import_data(folder)

#' # Merge Data
# ---- merge-data -------------------------------------------------------------

ds_obesity <- bind_rows(ds_obesity_raw) %>% 
  select(-county, -state)

#' # Save to Disk
# ---- save-data --------------------------------------------------------------

ds_obesity %>% write_rds(
  "./data-public/derived/us-obesity-data.rds"
  ,compress = 'gz'
  )
